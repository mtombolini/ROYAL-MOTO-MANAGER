...
class ProductExtractor(DataExtractor):
    ...
    def get_data(self):
        while True:
            endpoint = (
                f"variants.json?limit={self.limit}"
                f"&offset={self.offset}"
                f"&expand=[product_type]"
            )
    ...
    def write_product_logs(self):
        with open("logs/api_status.log", "a") as log_file:
            message = json.dumps({
                "tipo": "productos",
                "mensaje": f"{self.offset} productos obtenidos"
            })
    ...
    def get_stocks(self):
        self.offset = 0
        while True:
            endpoint = f"stocks.json?limit={self.limit}&\
            offset={self.offset}&expand=[office]"
            endpoint = (
                f"stocks.json?limit={self.limit}"
                f"&offset={self.offset}"
                f"&expand=[office]"
            )
            response = self.make_request(endpoint)
    ...
    def write_stock_logs(self):
        with open("logs/api_status.log", "a") as log_file:
            message = json.dumps({
                "tipo": "stocks",
                "mensaje": f"{self.offset} stocks obtenidos"
            })
            log_file.write(message + "\n")

    def order_stocks_dataframe(self):
        pivot_df = self.df_stocks.pivot_table(
            index='Variant ID',
            columns='Office',
            values='Quantity',
            fill_value=0
    ...
    def filter_products_by_stock(self):
        ...
        for index, row in self.df_products.iterrows():
            ...
            variant_in_existing = variant_id not in existing_variant_ids
            row_in_services = row['Product Type'] in services
            if variant_in_existing and row_in_services:
                new_rows.append({
        ...
        if new_rows:
            self.df_stocks = pd.concat(
                [self.df_stocks, pd.DataFrame(new_rows)],
                ignore_index=True
            )

        variant_ids = self.df_stocks['Variant ID'].values
        self.df_products = self.df_products[
            self.df_products['Variant ID'].isin(variant_ids)
        ]
